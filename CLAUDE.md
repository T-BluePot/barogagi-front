...
### 작성 원칙
- **본문을 길게 적지 않는다.** 근거를 장문으로 늘어놓지 말 것.
- **설명이 길어질 것 같으면 커밋 단위를 쪼갠다.** 한 커밋에 여러 변경을 몰아넣고 본문으로 해설하는 대신, 변경마다 커밋을 나눈다. 코드 리뷰 수정도 지적 건별로 나눈다.
- **제목에는 무엇을 고쳤는지만 적는다.** "~에 대한 지적 반영", "코드리뷰 N차" 같은 표현을 쓰지 않는다.
- **제목과 본문 모두 명사형으로 끝낸다.** (예: `~에 대해 수정`, `~추가`)
- **긴 대시(—)를 쓰지 않는다.**

근거는 코드 주석이나 PR 코멘트에 남긴다. 커밋은 변경 단위와 제목으로 말하게 한다.

...
> 브릿지 명세·RN 코드·체크리스트는 [`docs/RN_BRIDGE.md`](./RN_BRIDGE.md) §13 에 있다. 아래는 요약이다.
...
API 를 직접 호출해 확인한 결과다. 스웨거·API 문서에 없던 내용이라 여기 남긴다.

| 확인한 것 | 결과 |
| --- | --- |
| 다중 기기 로그인 | **가능**. `deviceId` 가 다르면 두 세션이 동시에 살아 있다 |
| 로그아웃 | **그 기기만** 끊긴다 (A 로그아웃 후 A 401 / B 200). 문서 명세대로 |
| 탈퇴 | **모든 기기**가 끊긴다 (A 로 탈퇴 후 A·B 둘 다 401). 로그아웃과 다르다 |
| 유예 중 로그인 | **탈퇴가 철회된다**. 재로그인이 `L200` 으로 성공하고 계정이 복구된다 |

→ 다른 기기에서 탈퇴하면 이 기기는 401 을 받아 강제 로그아웃 경로를 타고,
  로그인 화면에서 로컬 등록 기록이 비워진 뒤 재로그인 시 재등록된다.
  클라이언트가 "다른 기기의 탈퇴"를 알 방법은 이 401 뿐이다.

⚠️ 탈퇴 사유는 `reasonNo` 를 `/api/v1/withdrawal-reasons` 에서 받아 써야 한다.
   임의 번호(예: `1`)는 `D401` 로 거절된다. `essentialYn: "Y"` 인 사유(기타)는
   `withdrawReason` 없이 보내면 `D402` 로 거절된다. 둘 다 API 문서의 에러 코드 목록에 없다.

## 검증 현황

**완료** — 브라우저: deviceId 4경로(정상/최초/구버전/저장소 실패), 로그아웃·탈퇴·로테이션·승격·레거시 정리
**완료** — 실기기(폰 브라우저): `crypto` 폴백, deviceId 생성·유지, 로그인 통과, 로그아웃 후 식별자 생존
**완료** — test 서버 API 직접 호출: 위 "서버 동작 실측" 표
